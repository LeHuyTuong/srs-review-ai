...
void main() {
  test('locate cited captions across all pages', () async {
    final doc = await ParseService().parse(
      bytes: Uint8List.fromList(await File(Platform.environment['SRS_TEST_PDF']!).readAsBytes()),
      fileName: 'otes.pdf',
    );
    for (final needle in ['Table 40', 'Figure 62', 'Figure 63', 'Figure 61', 'Save student', "student's video"]) {
      final pages = <int>[];
