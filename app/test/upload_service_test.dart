...
class _ScriptedAdapter implements HttpClientAdapter {
  ...
  Future<ResponseBody> fetch(
    ...
    requests.add(
      (method: options.method, uri: options.uri, data: options.data),
    );
...
void main() {
  ...
  group('UploadService.uploadDocument', () {
    test('presign then PUT the exact bytes to the resolved token URL', () async {
      final adapter = _ScriptedAdapter(
        Queue.of([
          ...
          _json(
            {'key': 'abc123-otes.pdf', 'size': 3, 'sha256': 'deadbeef'},
            status: 201,
          ),
        ]),
      );
      ...
      expect(put.uri, Uri.parse('http://127.0.0.1:8000/uploads/abc123-otes.pdf?token=TOK.ENCRYPTED'));
    ...
    test('base path with trailing slash does not double the separator', () async {
      final adapter = _ScriptedAdapter(
        Queue.of([
          _json({
