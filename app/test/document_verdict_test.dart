...
List<DeterministicFinding> _allFloorPass({
  List<DeterministicFinding> extra = const [],
}) => [
...
void main() {
  group('computeVerdict (rubric Mục E)', () {
    ...
    test(
      'clean diagrams + clean naming: 9/10 with traceability unassessed',
      () {
        final v = computeVerdict(
          _allFloorPass(
            extra: [
    ...
    test(
      'diagram high row forfeits the +2 but DOC-family reds do not deduct',
      () {
        final v = computeVerdict(
          _allFloorPass(
            extra: [
    ...
    test('no family named FLOW is ever emitted, so none may be relied on', () {
      ...
      for (final prefix in deductingFamilies) {
        expect(
          ...
          isTrue,
          reason:
              '$prefix matches no family DiagramKind can produce — '
    ...
    test(
      'crossArtifact failure forfeits only its own bonus, not the floor',
      () {
        final v = computeVerdict(
          _allFloorPass(
            extra: [
