...
**version:** 1.7-draft · 2026-09-23 · **CHỜ AMY DUYỆT** (1.5 là bản LOCKED gần nhất). 1.7 thêm **sáu luật mới** ở `references/quality-rules.md` §F — trang bìa đủ thông tin (`coverPageInfo`), header/footer nhất quán (`headerFooterConsistency`), khai báo form khớp trang bìa (`projectInfoMismatch`), thứ tự chương theo mục lục (`sectionOrder`), heading số phân cấp (`headingNumbering`) và số trang ở dòng cuối (`pageNumbering`), tất cả deterministic thuần text; **không đổi luật cũ, không đổi thang điểm, không đổi gate** (furniture chưa có thành phần điểm — finding chỉ là tín hiệu ledger, luật một-chỗ §6). Adapter và cache đọc dòng này.
...
│   ├── quality-rules.md         9(+1) tính chất requirement, ISO 25010, ambiguity scan, §F trang bìa/header-footer
...
| **1.7-draft** | 2026-09-23 | **Sáu luật mới, tất cả thuần text** — `quality-rules.md` §F: trang bìa đủ 3 trường (`coverPageInfo`: thiếu tên đề tài red, thiếu GVHD amber, thiếu nhóm info), header/footer nhất quán (`headerFooterConsistency`: info, biến thể phải khác CHỮ không chỉ số), khai báo form khớp trang bìa (`projectInfoMismatch`: chỉ khi form đã khai — title khớp < 50% token → high, GVHD không thấy nguyên chuỗi trên bìa → medium), thứ tự chương (`sectionOrder`: chương bắt đầu trước khi chương trước kết thúc → medium, chỉ khi index trusted), và §F.5 — heading số thiếu phân cấp/trùng số (`headingNumbering` → medium, chỉ unit `SEC-<số>`), số trang vắng/lộn xộn ở dòng cuối trang (`pageNumbering` → low/heuristic text layer, luôn dặn kiểm tra bằng mắt; gate ≥ 6 trang). Động cơ: SRS sinh viên thiếu/không nhất quán furniture (tên đề tài, GVHD, nhóm, đánh số) — lỗi không nằm trong requirement nào nên mọi check theo-requirement và LLM pass đều mù; còn §F.4 đọc chính mục lục của tài liệu. Severity cố ý thấp ở F.2/F.5b vì false positive từ running head và từ việc extractor để footer giữa text; cả sáu đều ghi rõ giới hạn heuristic + không OCR. Phần format còn lại (font, cỡ chữ, căn lề, giãn dòng, caption tồn tại) **chưa port** — cần span metric/bbox từ `docmap.py`, ghi ở port-map. **Chưa đo độ lặp lại, chưa đổi thang điểm** — port app ở `adapters/app-port-map.md` §1 |
