...
| §F.1 Trang bìa đủ 3 trường (tên đề tài / GVHD / nhóm) | ✅ `coverPageInfo` | Dart `header_footer_checks.dart` | heuristic NHÃN trường trên 2 trang đầu (DOCX: 2000 ký tự đầu); bìa scan/im tên không nhãn là giới hạn đã biết — ghi trong §F.1 |
| §F.2 Header/footer không nhất quán giữa các trang | ✅ `headerFooterConsistency` | Dart `header_footer_checks.dart` | 4 slot đầu/cuối trang; cặp biến thể recurring khác **chữ** (không chỉ số) → low; running head theo chương là false positive đã biết; cần ≥ 6 trang, không OCR |
| §F.3 Khai báo dự án khớp trang bìa (tên đề tài, GVHD) | ✅ `projectInfoMismatch` | Dart `project_info_checks.dart` | chỉ chạy khi form đã khai báo; title khớp < 50% token → high, GVHD không thấy nguyên chuỗi → medium; bìa < 3 dòng im lặng; false positive đã biết: tên viết tắt / từ đồng nghĩa |
| §F.4 Chương theo đúng thứ tự theo mục lục | ✅ `sectionOrder` | Dart `project_info_checks.dart` | chỉ khi `blueprint.trusted`; `printedStart` chương sau < `printedEnd` chương trước → medium; KHÔNG phán "đủ khung mẫu" (việc của `missingSection`) |
| §F.5a Heading số thiếu phân cấp / trùng số | ✅ `headingNumbering` | Dart `format_layout_checks.dart` | chỉ unit `SEC-<số>` (parser đã tách heading); thiếu tiền tố → medium, trùng chuỗi số → medium; heading không đánh số im lặng; so segment từng cấp, không so số thập phân |
| §F.5b Số trang không thấy ở dòng cuối trang | ✅ `pageNumbering` | Dart `format_layout_checks.dart` | gate ≥ 6 trang; dòng cuối pageTexts = số standalone/`Trang n`; 0 trang có số → low, số lặp/giảm ≥ 3 chỗ → low; text layer ≠ layout → false positive đã biết → message luôn dặn kiểm tra bằng mắt; scan im lặng |
| §F.5 phần còn lại: font, cỡ chữ, căn lề, giãn dòng, khoảng trắng, caption tồn tại | ❌ **cần server** | `docmap.py` (span font/size + figure bbox ↔ dòng caption) | text parser không có metric — port giả tệ hơn không port; giữ ở ledger người-reviewer tới khi docmap trả span data |
| §F.6 Bảng bị dời ra xa vị trí mục lục khai báo | ✅ `tablePositionDrift` | Dart `blueprint_builder.dart` + `blueprint_checks.dart` | chỉ khi `blueprint.trusted`; builder quét toàn thân khi cửa sổ ±3 hụt → ghi `foundPageIndex` (không đụng `pdfPageIndex`); lệch > cửa sổ → medium, nêu 2 trang + độ lệch + chương thật vs chương mục lục; `captionPageMismatch` bỏ qua artifact có `foundPageIndex`; không LoT / DOCX / bảng không caption → im lặng |
| §F.6 Bảng/hình bị dời ra xa vị trí mục lục khai báo | ✅ `tablePositionDrift` | Dart `blueprint_builder.dart` (global search → `foundPageIndex`) + `blueprint_checks.dart` | chỉ khi `blueprint.trusted`; lệch > cửa sổ ±3 → medium nêu 2 trang + độ lệch + chương thật vs chương mục lục; `captionPageMismatch` bỏ qua artifact có `foundPageIndex`; không LoT / DOCX / bảng không caption → im lặng; caption probe trước label probe (chống tham chiếu chéo) |
