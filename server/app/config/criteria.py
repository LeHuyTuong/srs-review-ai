...
from ..contracts.schemas import IssueType
...
def validate(criterion: dict[str, Any], where: str = "criterion") -> None:
    ...
    if severity not in VALID_SEVERITIES:
        raise ValueError(f"{where}: severity must be one of {VALID_SEVERITIES}, got {severity!r}")
...
class CriteriaStore:
    ...
    def enabled(self, scope: str | None = None) -> list[dict[str, Any]]:
        return [row for row in self.list() if row["enabled"] and (scope is None or row["scope"] == scope)]
    ...
    def stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "unit_scope": sum(1 for row in rows if row["enabled"] and row["scope"] == "unit"),
            "document_scope": sum(1 for row in rows if row["enabled"] and row["scope"] == "document"),
    ...
    def update(self, criterion_id: str, payload: dict[str, Any]) -> dict[str, Any] | None:
    ...
    def delete(self, criterion_id: str) -> bool:
        ...
        if self._conn is None:
            ...
            return True
        self._write(lambda conn: conn.execute("DELETE FROM criteria WHERE id = ?", (criterion_id,)))
        return True
    ...
    def prompt_block(self, scope: str) -> str:
        ...
        for index, row in enumerate(rows, start=1):
            lines.append(f"  {index}. [{row['id']}] {row['title']} (default severity {row['severity']})")
